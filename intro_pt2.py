...
class IntroPtII(Scene):
    def construct(self):
        ...
        grandGroup = VGroup(grand, defGrand)
        self.play(grandGroup.animate.to_edge(UP, buff=0.85))
        self.wait(1)
        med = Text("Medida:", font_size=36)
        self.play(Write(med))
        self.play(med.animate().scale(0.8).shift(LEFT * 6))
        self.play(med.animate.next_to(grand, DOWN, buff=0.66))
        self.wait(1)
        ...
        self.wait(0.5)
        self.play(msg.animate.shift(UP * 1.25).scale(0.88))
        self.wait(2.5)
        self.play(FadeOut(msg), Unwrite(grandGroup), Unwrite(med, reverse=False, remover=True), Unwrite(defMed, reverse=True, remover=False))
        self.wait(0.5)
        ...
        self.wait(1)
        for i in range(1, len(col1_texts)):
            self.play(Uncreate(col1_texts[i]))
            self.wait(0.5)
            self.play(Uncreate(col2_texts[i]))
            self.wait(0.5)
        self.play(Unwrite(titulo_col1_start))
        self.play(Unwrite(titulo_col2_start))
        self.wait(1)